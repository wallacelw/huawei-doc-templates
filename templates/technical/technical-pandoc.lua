...
local function parse_preamble(source_text)
  ...
  -- Parse \documentclass[opts]{technical}
  local opts = source_text:match("\\documentclass%s*%[(.-)%]%s*{technical}")
  if opts then
...
function Pandoc(doc)
  ...
  local title = preamble.commands.setreporttitle
  if title then
  ...
  local version = preamble.commands.setreportversion
  if version then doc.meta["doc-version"] = version end
  local date = preamble.commands.setreportdate
  ...
  if FORMAT:match("docx") then
    local ct = "Huawei Technologies CO., LTD"
    local meta_parts = {}
    -- Per technical.cls: cover meta (version + date + time) only shown when changelog is enabled
    ...
    local cover_logo = "huawei-logo-cover.png"
...
function RawBlock(raw)
  ...
  -- Anchor to start of text so nested environments don't steal the block
