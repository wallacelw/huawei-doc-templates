...
--- Each template wrapper calls make_filter with template-specific config,
--- then assigns the returned Pandoc/RawBlock/RawInline/Header/CodeBlock
--- as globals (NOT a return table — Pandoc auto-discovers globals only).
---
...
local function make_filter(config)
  ...
  local strip_commands = config.strip_commands
  ...
  local function parse_preamble(source_text)
    ...
    -- Parse \documentclass[opts]{class_name}
    local opts = source_text:match("\\documentclass%s*%[(.-)%]%s*{" .. config.class_name .. "}")
    if opts then
  ...
  if type(config.extra_env_handlers) == "function" then
    ...
    for env, handler in pairs(config.extra_env_handlers or {}) do
      block_env_handlers[env] = handler
    end
  end
  ...
  local function Pandoc_fn(doc)
    -- Warn about PDF-only commands (guide only)
    if config.warn_setheaderlogo then
    ...
    local title = preamble.commands[config.command_map.title]
    if title then
    ...
    local version = preamble.commands[config.command_map.version]
    if version then doc.meta["doc-version"] = version end
    local date = preamble.commands[config.command_map.date]
    ...
    if FORMAT:match("docx") then
      local ct = config.cover_text or preamble.commands.setcovertext or "Huawei Technologies CO., LTD"
      local meta_parts = {}
      ...
      local cover_logo = config.cover_logo or preamble.commands.setcoverlogo or "huawei-logo-cover.png"
  ...
  -- =====================================================================
  --  DOCX Header function
  -- =====================================================================

  local function Header_fn(el)
    if not FORMAT:match("docx") then return nil end
    if el.level > 4 then return nil end
  ...
  -- Strip syntax highlighting classes for DOCX
  local function CodeBlock_fn(el)
  ...
  local function RawBlock_fn(raw)
  ...
  local function RawInline_fn(raw)
    if raw.format ~= "latex" then return nil end
    local text = raw.text
  ...
  inner_filter = { RawInline = RawInline_fn, RawBlock = RawBlock_fn }

  return {
    Pandoc = Pandoc_fn,
    RawBlock = RawBlock_fn,
...
return { make_filter = make_filter }
