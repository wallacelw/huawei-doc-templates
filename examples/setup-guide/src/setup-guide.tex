...
\begin{enumerate}
  ...
  \item On the Flexus X instance, remove the document templates installation:
  \begin{code}[bash]
cd /home/huawei-doc-templates
./uninstall.sh --all --yes
  \end{code}
  \begin{tip}
  Use \inlinecode{--dry-run} first to preview what will be removed. Add
  \inlinecode{--packages} to also remove apt packages (texlive, latexmk, fonts),
  but this will break any other LaTeX documents on the instance.
  \end{tip}
\end{enumerate}
