...
\setdoctitle{Guide: Environment Setup for Huawei Cloud Development}
...
\stepbystep
\begin{enumerate}
  \item Run the one-liner install (clones the repo to
        \inlinecode{/home/huawei-doc-templates} and installs everything):
...
curl -fsSL https://raw.githubusercontent.com/wallacelw/huawei-doc-templates/main/scripts/install.sh | bash
  ...
  \begin{infobox}
  The script installs XeLaTeX, latexmk, fonts (HarmonyOS Sans + Cascadia Code),
  the opencode skills, and configures VS Code LaTeX Workshop. When run
  interactively (\inlinecode{./scripts/install.sh}), it prompts for optional components
  ...
  \inlinecode{Update v3.3.6 → v3.3.7? [Y/n]}. It pulls the latest version and
  re-runs the installer. No need to \inlinecode{git pull} manually.
  \end{infobox}
...
\begin{objectives}
  \generalobjective{Update, uninstall, or clean up the software and cloud
  resources created in this guide. All operations in this chapter are optional.}
\end{objectives}

\subsection{Updating the MaaS Gateway}

To update the oh-my-coding-maas-gateway to a newer version:

\stepbystep
\begin{enumerate}
  \item Re-run the bootstrap script in the VS Code remote terminal:
  \begin{code}[bash]
curl -fsSL https://raw.githubusercontent.com/wallacelw/oh-my-coding-maas-gateway/main/scripts/bootstrap.sh | bash
  \end{code}
  \item The installer detects the existing installation, pulls updates, and
        preserves all secrets, virtual keys, and data.
  \item To update only the coding tools (without touching the proxy stack):
  \begin{code}[bash]
...
\subsection{Updating the Document Templates}

To update the Huawei document templates project:

\stepbystep
\begin{enumerate}
  \item Re-run the one-liner --- it detects the existing installation and
        prompts to update:
  \begin{code}[bash]
curl -fsSL https://raw.githubusercontent.com/wallacelw/huawei-doc-templates/main/scripts/install.sh | bash
  \end{code}
  \item The script pulls the latest version, shows the version change
        (\inlinecode{v3.3.6 → v3.3.7}), and re-runs the installer. New
        templates, commands, or modules are installed automatically.
\end{enumerate}

\subsection{Uninstalling the MaaS Gateway}

To remove the oh-my-coding-maas-gateway:

\stepbystep
\begin{enumerate}
  \item Run the uninstall script:
  \begin{code}[bash]
cd /home/oh-my-coding-maas-gateway
./scripts/uninstall.sh --all --yes
  \end{code}
  \item This stops and removes Docker containers, volumes, and images, removes
        coding tool configs (opencode, Codex, Claude, Pi), and deletes the
        repository.
  \item To remove only specific components:
  \begin{code}[bash]
...
\subsection{Uninstalling the Document Templates}

To remove the Huawei document templates installation:

\stepbystep
\begin{enumerate}
  \item Run the uninstall script (interactive menu with 5 options):
  \begin{code}[bash]
cd /home/huawei-doc-templates
./scripts/uninstall.sh
  \end{code}
  \item Or use the one-liner:
  \begin{code}[bash]
curl -fsSL https://raw.githubusercontent.com/wallacelw/huawei-doc-templates/main/scripts/uninstall.sh | bash
  \end{code}
  \item The interactive menu offers:
  \begin{itemize}
    \item Option 1: All installed components (skills, modules, font, VS Code)
    \item Option 2: Everything + apt packages (WARNING: breaks other TeX)
    \item Option 3: Delete repository directory (all files, guides, documents)
    \item Option 4: Remove 100\% --- everything + apt + repo (nuclear option)
    \item Option 5: Choose specific components individually
  \end{itemize}
  \item To preview what would be removed before running:
  \begin{code}[bash]
./scripts/uninstall.sh --all --dry-run
  \end{code}
  \item To also remove apt packages (texlive, latexmk, fonts, pandoc):
  \begin{code}[bash]
./scripts/uninstall.sh --all --packages --yes
  \end{code}
  \begin{warning}
  Removing apt packages will break any other LaTeX documents on the instance.
  Only use \inlinecode{--packages} if you are decommissioning the instance.
  \end{warning}
\end{enumerate}

\subsection{Deleting cloud resources (optional)}
...
\begin{table}[H]
  \begin{hutable}{|l|l|l|}
    ...
    Templates update & \inlinecode{git pull \&\& ./scripts/install.sh}     & New templates available \\
    Gateway uninstall & \inlinecode{./scripts/uninstall.sh --all}  & Docker stack removed \\
    Templates uninstall & \inlinecode{./scripts/uninstall.sh --all}         & Skills + modules removed \\
    Cloud cleanup    & \inlinecode{flexusx-dev} deleted            & No running instance \\
...
\begin{hutable}{|l|l|l|}
\rowcolor{huaweired} \thd{Color} & \thd{HEX} & \thd{Brightness} \\
\tbody
\textcolor{huaweiblack}{Black} & \texttt{\#000000} & 0\% \\
\textcolor{huaweigray90}{Gray 90\%} & \texttt{\#E5E5E5} & 90\% \\
\textcolor{huaweigray80}{Gray 80\%} & \texttt{\#CCCCCC} & 80\% \\
\textcolor{huaweigray50}{Gray 50\%} & \texttt{\#808080} & 50\% \\
\textcolor{huaweigray30}{Gray 30\%} & \texttt{\#4D4D4D} & 30\% \\
\textcolor{huaweiwhite}{White} & \texttt{\#FFFFFF} & 100\% \\
\end{hutable}
