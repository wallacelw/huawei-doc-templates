...
curl -fsSL https://raw.githubusercontent.com/wallacelw/huawei-doc-templates/main/install.sh | bash
  ...
  the opencode skills, and configures VS Code LaTeX Workshop. It is idempotent
  --- safe to re-run. It runs \inlinecode{apt-get install} for TeX Live packages;
  if you do not have sudo access, ask your administrator to install them.
  \end{infobox}
...
\stepbystep
\begin{enumerate}
  \item Pull the latest changes and re-run the installer (idempotent):
  \begin{code}[bash]
cd /home/huawei-doc-templates
git pull
./install.sh --yes
  \end{code}
  \item The installer skips components that are already up to date. New
...
./uninstall.sh --all --yes
  \end{code}
  \item This removes: opencode skills, \inlinecode{.sty} modules from TDS,
        HarmonyOS Sans font, \inlinecode{/etc/LatexMk} fix, and VS Code
        settings + extensions.
...
% =====================================================================
\begin{changelog}
  \changelogentry{3.3.2}{\today}{\item Replaced manual \inlinecode{git clone} + \inlinecode{./install.sh} steps in Chapter 6 with one-liner \inlinecode{curl | bash} command.}
