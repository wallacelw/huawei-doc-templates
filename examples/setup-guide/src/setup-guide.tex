...
\begin{objectives}
  \generalobjective{Learn practical tips for working efficiently with the
  Huawei Cloud development environment, including persistent terminal
  sessions with tmux.}
\end{objectives}

\subsection{Using tmux for persistent sessions}

When working over SSH, your opencode session is tied to the SSH connection.
If the connection drops (network issue, VPN timeout, laptop sleep), the
opencode process is killed and all context is lost. \textbf{tmux} solves
this by decoupling the terminal from the SSH connection --- the session
keeps running on the server even when you disconnect, and you can reattach
to it when you reconnect.

\stepbystep
\begin{enumerate}
  \item Install tmux (if not already installed):
  \begin{code}[bash]
sudo apt install tmux
  \end{code}

  \item Start a new tmux session before launching opencode:
  \begin{code}[bash]
tmux new -s opencode
  \end{code}

  \item Inside the tmux session, start opencode as usual:
  \begin{code}[bash]
opencode
  \end{code}

  \item If SSH disconnects, the opencode session keeps running. Reconnect
        to the server and reattach:
  \begin{code}[bash]
tmux attach -t opencode
  \end{code}

  \item To list existing sessions:
  \begin{code}[bash]
tmux ls
  \end{code}
\end{enumerate}
...
\begin{tip}
Always start your opencode session inside tmux. If you forget and start
opencode directly, you can't retroactively attach it to tmux --- you'd
need to restart opencode inside a new tmux session.
\end{tip}

\subsubsection{Essential tmux commands}

\begin{table}[H]
  \begin{hutable}{|l|l|}
    \rowcolor{huaweired} \thd{Command} & \thd{Purpose} \\
    \tbody
    \inlinecode{tmux new -s <name>}   & Start a new named session \\
    \inlinecode{tmux attach -t <name>} & Reattach to an existing session \\
    \inlinecode{tmux ls}              & List all sessions \\
    \inlinecode{tmux kill-session -t <name>} & Kill a specific session \\
    \inlinecode{Ctrl-b d}             & Detach from session (keeps it running) \\
    \inlinecode{Ctrl-b c}             & Create a new window \\
    \inlinecode{Ctrl-b \%}            & Split pane vertically \\
    \inlinecode{Ctrl-b "}             & Split pane horizontally \\
    \inlinecode{Ctrl-b n}             & Next window \\
    \inlinecode{Ctrl-b p}             & Previous window \\
    \inlinecode{Ctrl-b [}             & Enter scroll/copy mode \\
  \end{hutable}
  \caption{Essential tmux commands.}
\end{table}

\subsubsection{tmux configuration for opencode}

Create or edit \inlinecode{\textasciitilde/.tmux.conf} with the following
settings to optimize tmux for use with opencode:
...
\begin{warning}
The \inlinecode{escape-time} setting is critical. The default 500ms delay
causes opencode to receive Escape key presses late, which breaks modal
editing and menu navigation. Always set it to \inlinecode{10} or lower.
\end{warning}

\begin{warning}
The \inlinecode{extended-keys-format csi-u} setting requires \textbf{tmux 3.5
or later}. If you get an \inlinecode{invalid option: extended-keys-format}
error, your tmux is too old. Check with \inlinecode{tmux -V}. To upgrade
tmux to 3.5+ from source:
\begin{code}[bash]
...
\end{code}
After upgrading, remove the old binary from \inlinecode{/usr/bin/tmux} if
needed (\inlinecode{which tmux} should point to \inlinecode{/usr/local/bin/tmux}).
Then reload the config: \inlinecode{tmux source-file ~/.tmux.conf}.
\end{warning}

\begin{tip}
If you cannot upgrade tmux, use \inlinecode{Ctrl+J} to insert a newline
in opencode --- it works with zero configuration on any tmux version.
\end{tip}

\begin{infobox}
For more tips on using tmux with opencode, including WSL2 clipboard
integration and keybinding workflows, see
\weblink{https://lukemanning.ie/blog/opencode-tmux-getting-used-to-keybindings}{Luke Manning's blog post}.
\end{infobox}
...
\begin{table}[H]
  ...
  \end{hutable}
  \caption{Chapter 8 verification checklist.}
\end{table}
...
% =====================================================================
\begin{changelog}
  \changelogentry{3.8.0}{\today}{\item Added Chapter 7: Tips and Tricks with tmux for persistent SSH sessions, essential tmux commands, and tmux configuration for opencode (escape-time, mouse, clipboard, extended-keys for Shift+Enter).}
