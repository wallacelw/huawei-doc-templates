...
:   -   Terraform infrastructure apply completed successfully. - MRS
...
:   -   Cluster status is **Running**. - All four components (HDFS,
        YARN, Spark, Hive) are listed with status **Normal**. - All
        nodes report status **Running** with no alarms.
...
:   -   OBS bucket name is defined in the Terraform configuration. - IAM
        policy granting read/write access to the bucket is attached to
        the test user.
...
:   -   Bucket exists with storage class **Standard**. - File upload
        completes without error. - Downloaded file content matches the
        original. - File deletion succeeds.
...
:   -   VPC and subnet IDs are available from the Terraform output. -
        Security group names are documented in the solution
        architecture.
...
:   -   VPC CIDR is `10.0.0.0/16`. - Subnet CIDR is `10.0.1.0/24`. -
        Inbound rules match the architecture: TCP 22 and TCP 443 from
        the specified sources. - Outbound allows all traffic.
...
:   -   DataArts Studio instance is provisioned and its status is
        **Running**. - Test IAM user has the **Dayu_User** role
        assigned.
...
:   -   Workspace is listed and accessible. - DataArts Studio console
        opens without errors. - All three modules (Data Integration,
        Data Development, Data Architecture) are visible in the
        navigation.
...
:   -   TC-004 completed (DataArts Studio workspace accessible). -
        Source CSV file (`customers.csv`) exists in the OBS raw
        bucket. - Target OBS path (`poc-datalake-curated/customers/`) is
        configured. - Pipeline `pl-ingest-customers` is published in
        DataArts Studio.
...
:   -   Pipeline execution completes with status **Success**. - Output
        file is created in the target OBS path. - Row count of the
        output file matches the source file.
...
:   -   TC-001 completed (MRS cluster healthy). - Hive table
        `poc_db.customers` exists and contains data loaded by the
        ingestion pipeline. - SSH access to the MRS master node is
        configured.
...
:   -   `COUNT(*)` returns 10,000. - The sample query returns 5 rows
...
:   -   OBS bucket `poc-datalake-raw` exists with the designated IAM
        policy. - Two test IAM users: one with **OBS_ReadWrite** role,
        one with no OBS permissions. - `obsutil` CLI configured for both
        users.
...
:   -   Authorized user can upload and download successfully. -
...
:   -   TC-004 completed (DataArts Studio workspace accessible). - Data
        masking rule configured for column `customers.ssn` (Social
        Security Number). - Test user has **Dayu_User** role (not
        admin).
...
:   -   Masking rule is active for `customers.ssn`. - Non-admin user
        sees masked values in the `ssn` column. - Admin user sees real
...
:   -   `terraform` CLI installed (version $\geq$ 1.0). - AK/SK
        credentials configured via environment variables or
        *provider.tf*. - Terraform configuration files in the project
        directory.
...
:   -   `terraform apply` completes with no errors. - All output values
        are populated (cluster ID, bucket name, VPC ID). - Resource
        count in state matches the architecture specification.
...
:   -   TC-002 completed (OBS bucket accessible). - Test data files
        prepared in the local directory *./test-data/*. - `obsutil` CLI
...
:   -   All test data files are uploaded without errors. - Bucket
        listing shows all uploaded files with correct sizes. -
        Downloaded file checksum matches the local original. - DataArts
        Studio data source connection can read the files.
