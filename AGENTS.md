...
- DOCX and Markdown are generated via Pandoc from `.adoc` (or
  asciidoctor-reducer → pandoc). HTML is generated via asciidoctor with
  `huawei.css`.
- `make all-formats` generates all outputs for all samples.
- Generated outputs (`.tex`, `.docx`, `.md`, `.html`) are gitignored (build
  artifacts). Only the converter, reference DOCX, HTML template, CSS, and
  Python script are committed.
...
### L21. AsciiDoc (.adoc) is the source of truth. LaTeX (.tex) is generated, not hand-edited.
- All document content is written in AsciiDoc (`.adoc`) files.
- LaTeX (`.tex`) files are generated by `huawei-latex-converter.rb` during
  the build process and are gitignored build artifacts.
- Never hand-edit a generated `.tex` file — changes will be overwritten
  on the next build.
- The `.cls` and `.sty` files are still hand-maintained (they define the
...
- `:template: guide|technical|testbook` — selects the template class.
