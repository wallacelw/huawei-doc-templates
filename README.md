...
3. **AsciiDoc → HTML:** `asciidoctor -b html5 -a stylesheet=huawei.css` (after pre-processing)
4. **AsciiDoc → DOCX/MD:** via pre-processor → `asciidoctor -b docbook` → `pandoc -f docbook` → `docx_fix.py --fix` (DOCX only)
