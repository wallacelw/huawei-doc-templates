...
++++
\begin{changelog}
  \changelogentry{6.8.0}{2026-09-20}{\item Technical report covers now match the PDF in DOCX/MD/HTML: the title uses the PDF's 24pt size (was 36pt), the report type label appears above it, and the Version/Date/Scenario table is rendered with the PDF's red label column. The cover meta line now shows the report version and its set date (was the document version and the build date). Authors, when set, appear as a table row like the PDF.}
